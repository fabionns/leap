...
if (( $NODE == 0 )) ; then
    ...
    calico)
        wget -q https://raw.githubusercontent.com/projectcalico/calico/v3.25.0/manifests/tigera-operator.yaml -O /tmp/calico-default.yaml
        sed "s+192.168.0.0/16+$POD_CIDR+g" /tmp/calico-default.yaml > /tmp/calico-default.yaml
        kubectl apply -f /tmp/calico-default.yaml
        ;;
    weave)
